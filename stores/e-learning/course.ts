...
export const useCourseStore = defineStore('course-lesson', () => {
  ...
  const lessonMarkAsComplete = async (lessonUUID: string): Promise<void | NavigationFailure> => {
    try {
      const { data } = await $axiosApi.put(`/lessons/${lessonUUID}/mark-as-complete`)
  ...
  const lessonUnmarkAsComplete = async (lessonUUID: string): Promise<void | NavigationFailure> => {
    try {
      const { data } = await $axiosApi.put(`/lessons/${lessonUUID}/unmark-as-complete`)

      if (!data) throw new Error('Response Data Not Found!')

      if (lesson.value) lesson.value.is_completed = false
