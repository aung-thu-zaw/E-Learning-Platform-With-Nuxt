...
onMounted(async () => await store.getAllCategory(queryString.value))

watch(
  () => route.query,
  async () => await store.getAllCategory(queryString.value)
)
</script>
...
<template>
  <div class="space-y-5">
    ...
    <div class="flex flex-col items-start md:flex-row md:items-center md:justify-between mb-10">
      ...
      <!-- Create Button -->
      <div v-show="can('categories.create')" class="flex items-center justify-end mb-3">
        <NuxtLinkButton to="/admin/catalogues/categories/create">
    ...
    <div class="border border-gray-300 bg-white rounded-md shadow-sm shadow-gray-200 px-5 py-3">
          ...
          <template #table-data="{ item }">
            ...
            <TableActionCell>
              <NuxtLinkButton
                ...
                :to="'/admin/catalogues/categories/' + item?.slug + '/edit'"
              >
              ...
              <NormalButton
                ...
                @click="store.deleteCategory(item?.slug)"
              >
      ...
      <div v-if="categories?.data?.length" class="my-10">
