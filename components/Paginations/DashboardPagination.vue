...
<template>
  ...
  <!-- Meta Pagination -->
  <div v-if="data?.meta" class="flex items-center justify-center w-full">
  ...
  <!-- No Meta Pagination -->
  <div v-else-if="!data?.meta" class="flex items-center justify-center w-full">
        ...
        <template v-for="(link, p) in data?.links" :key="p">
          ...
          <div v-else class="flex items-center">
            <button
              ...
              :class="{
                'bg-yellow-500 text-white hover:bg-yellow-400 border-yellow-500':
                  Number(link.label) === data?.current_page
              }"
              :href="link?.url"
              @click="fetchData(link.url)"
              v-html="link.label"
