...
const getAllUserInterestedTag = async () => {
  try {
    ...
    if (data?.length) handleSelectTag(data[0])
  } catch (error: any) {
    console.error(error)
  }
}
...
onMounted(async () => {
  ...
  if (isAuthenticated.value) await getAllUserInterestedTag()
})
</script>

<template>
  <section v-if="isAuthenticated" class="py-10">
    <div class="container mx-auto">
      <div v-if="followedTags?.length">
        ...
        <div class="w-full">
          <div class="space-y-10">
            <h1
              class="font-bold text-gray-800 text-3xl [text-shadow:_1.5px_0.5px_1.5px_rgb(0_0_0_/_40%)]"
            >
              {{ $t('Your Creative Interests') }}
            </h1>
            <div class="flex items-center justify-center">
              <p class="text-sm font-medium text-gray-800">
                {{ $t('Start following skills and they will show up here.') }}
              </p>
            </div>
          </div>
        </div>
